...
namespace Librainian.Measurement.Time {
    ...
    public struct Span : IEquatable<Span>, IComparable<Span>, IComparable<TimeSpan> {
        ...
        public Span(
            ...
            // //BUG the "while"s used below can probably be changed to "if"s because of the math,
            // which should work the first time.

            // while ( this.PlanckTimes.Value > PlanckTimes.InOneYoctosecond ) { var truncate =
            // this.PlanckTimes.Value / PlanckTimes.InOneYoctosecond; this.PlanckTimes -= truncate *
            // PlanckTimes.InOneYoctosecond; truncate.ThrowIfOutOfDecimalRange(); this.Yoctoseconds
            // += ( System.Decimal )truncate; }

            // while ( this.Yoctoseconds.Value > Yoctoseconds.InOneZeptosecond ) { var truncate =
            // Math.Truncate( this.Yoctoseconds.Value / Yoctoseconds.InOneZeptosecond );
            // this.Yoctoseconds -= truncate * Yoctoseconds.InOneZeptosecond; this.Zeptoseconds +=
            // truncate; }

            // while ( this.Zeptoseconds.Value > Zeptoseconds.InOneAttosecond ) { var truncate =
            // Math.Truncate( this.Zeptoseconds.Value / Zeptoseconds.InOneAttosecond );
            // this.Zeptoseconds -= truncate * Yoctoseconds.InOneZeptosecond; this.Attoseconds +=
            // truncate; }

            // while ( this.Attoseconds.Value > Attoseconds.InOneFemtosecond ) { var truncate =
            // Math.Truncate( this.Attoseconds.Value / Attoseconds.InOneFemtosecond );
            // this.Attoseconds -= truncate * Attoseconds.InOneFemtosecond; this.Femtoseconds +=
            // truncate; }

            // while ( this.Femtoseconds.Value > Femtoseconds.InOnePicosecond ) { var truncate =
            // Math.Truncate( this.Femtoseconds.Value / Femtoseconds.InOnePicosecond );
            // this.Femtoseconds -= truncate * Femtoseconds.InOnePicosecond; this.Picoseconds +=
            // truncate; }

            // while ( this.Picoseconds.Value > Picoseconds.InOneNanosecond ) { var truncate =
            // Math.Truncate( this.Picoseconds.Value / Picoseconds.InOneNanosecond );
            // this.Picoseconds -= truncate * Picoseconds.InOneNanosecond; this.Nanoseconds +=
            // truncate; }

            // while ( this.Nanoseconds.Value > Nanoseconds.InOneMicrosecond ) { var truncate =
            // Math.Truncate( this.Nanoseconds.Value / Nanoseconds.InOneMicrosecond );
            // this.Nanoseconds -= truncate * Nanoseconds.InOneMicrosecond; this.Microseconds +=
            // truncate; }

            // while ( this.Microseconds.Value > Microseconds.InOneMillisecond ) { var truncate =
            // Math.Truncate( this.Microseconds.Value / Microseconds.InOneMillisecond );
            // this.Microseconds -= truncate * Microseconds.InOneMillisecond; this.Milliseconds +=
            // truncate; }

            // while ( this.Milliseconds > Milliseconds.InOneSecond ) { var truncate =
            // Math.Truncate( this.Milliseconds.Value / Milliseconds.InOneSecond );
            // this.Milliseconds -= truncate * Milliseconds.InOneSecond; this.Seconds += truncate; }

            // while ( this.Seconds.Value > Seconds.InOneMinute ) { var truncate = Math.Truncate(
            // this.Seconds.Value / Seconds.InOneMinute ); this.Seconds -= truncate *
            // Seconds.InOneMinute; this.Minutes += truncate; }

            // while ( this.Minutes.Value > Minutes.InOneHour ) { var truncate = Math.Truncate(
            // this.Minutes.Value / Minutes.InOneHour ); this.Minutes -= truncate *
            // Minutes.InOneHour; this.Hours += truncate; }

            // while ( this.Hours.Value > Hours.InOneDay ) { var truncate = Math.Truncate(
            // this.Hours.Value / Hours.InOneDay ); this.Hours -= truncate * Hours.InOneDay;
            // this.Days += truncate; }

            // while ( this.Days.Value > Days.InOneWeek ) { var truncate = Math.Truncate(
            // this.Days.Value / Days.InOneWeek ); this.Days -= truncate * Days.InOneWeek;
            // this.Weeks += truncate; }

            // while ( this.Months.Value > Months.InOneYear ) { var truncate = Math.Truncate(
            // this.Months.Value / Months.InOneYear ); this.Months -= truncate * Months.InOneYear;
            // this.Years += truncate; }

            // if ( this.Months.Value < Months.InOneYear ) { break; } if ( normalize ) { }
