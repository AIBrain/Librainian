...
namespace Librainian.Collections {
    ...
    /// <remarks>
    /// <para>
    /// The methods <see cref="Add"/>, <see cref="Remove"/>, and <see cref="Element()"/> are based on the <see cref="Offer"/>, <see cref="Poll"/>, and <see cref="Peek"/> methods respectively but throw exceptions instead
    ...
    internal abstract class AbstractQueue<T> : AbstractCollection<T>, IQueue<T> //JDK_1_6
    {
        ...
        /// </summary>
        /// <returns>true if the <see cref="ICollection{T}"/> is read-only; otherwise, false. This implementation always return false as typically a queue should not be read only.</returns>
        ...
        /// <summary>
        /// Inserts the specified <paramref name="element"/> into this queue if it is possible to do so immediately without violating capacity restrictions. Throws an <see cref="InvalidOperationException"/> if no space is
        /// currently available.
        /// </summary>
        ...
        /// </summary>
        /// <remarks>When using a capacity-restricted queue, this method is generally preferable to <see cref="Add"/>, which can fail to insert an element only by throwing an exception.</remarks>
        /// <param name="element">The element to add.</param>
