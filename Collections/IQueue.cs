...
//
// "Librainian/IQueue.cs" was last cleaned by Rick on 2016/06/18 at 10:50 PM
...
namespace Librainian.Collections {
	...
	///         the other returns a special value. The latter form of the insert operation is designed
	///         specifically for use with capacity-restricted <see cref="IQueue{T}" /> implementations; in
	...
	///         supplied comparator, or the elements' natural ordering, and LIFO queues (or stacks) which
	///         order the elements LIFO (last-in-first-out). Whatever the ordering used, the head of the
	///         queue is that element which would be removed by a call to <see cref="Remove" /> or
	///         <see cref="Poll" />. In a FIFO queue, all new elements are inserted at the tail of the
	///         queue. Other kinds of queues may use different placement rules. Every
	///         <see cref="IQueue{T}" /> implementation must specify its ordering properties.
	///     </para>
	///     <para>
	///         The <see cref="Offer" /> method inserts an element if possible, otherwise returning
	///         <c>false</c>. This differs from the <see cref="ICollection{T}.Add" /> method, which can fail
	///         to add an element only by throwing an exception. The <see cref="Offer" /> method is designed
	///         for use when failure is a normal, rather than exceptional occurrence, for example, in
	///         fixed-capacity (or "bounded") queues.
	///     </para>
	///     <para>
	///         The <see cref="Remove" /> and <see cref="Poll" /> methods remove and return the head of the
	///         queue. Exactly which element is removed from the queue is a function of the queue's ordering
	///         policy, which differs from implementation to implementation. The <see cref="Remove" /> and
	///         Poll <see cref="Poll" /> methods differ only in their behavior when the queue is empty: the
	///         <see cref="Remove" /> method throws an exception, while the <see cref="Poll" /> method
	///         returns <c>false</c>.
	///     </para>
	///     <para>
	///         The <see cref="Element" /> and <see cref="Peek" /> methods return, but do not remove, the
	///         head of the queue.
	///     </para>
	///     <para>
	///         The <see cref="IQueue{T}" /> interface does not define the blocking queue methods, which are
	///         common in concurrent programming.
	///     </para>
	///     <para>
	///         <see cref="IQueue{T}" /> implementations generally do not define element-based versions of
	///         methods <see cref="Object.Equals(Object)" /> and <see cref="Object.GetHashCode" />, but
	///         instead inherit the identity based versions from the class object, because element-based
    ...
    {
		/// <summary>
		///     Gets the remaining capacity of a bounded queue or <see cref="Int32.MaxValue" /> if the
		///     queue is un-bounded.
		/// </summary>
        ...
        ///     <see cref="ICollection{T}.Add" />, which can fail to insert an element only by throwing
        ///     an exception.
        /// </remarks>
