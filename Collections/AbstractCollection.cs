...
namespace Librainian.Collections {
    ...
    /// <summary>
    ///     Serve as based class to be inherited by the classes that needs to implement both the <see cref="ICollection" /> and
    ///     the <see cref="ICollection{T}" /> interfaces.
    /// </summary>
    /// <remarks>
    ///     <para>
    ///         By inheriting from this abstract class, subclass is only required to implement the
    ///         <see cref="GetEnumerator()" /> to complete a concrete read only collection class.
    ///     </para>
    ///     <para>
    ///         <see cref="AbstractCollection{T}" /> throws <see cref="NotSupportedException" /> for all access to the
    ///         collection mutating members.
    ///     </para>
    /// </remarks>
    ...
    public abstract class AbstractCollection<T> : ICollection<T>, ICollection //NET_ONLY
    {
        ...
        /// </summary>
        /// <remarks>
        ///     If <typeparamref name="T" /> is <see cref="Object" /> but array is actually <c>String[]</c>, the returned
        ///     array is always <c>String[]</c>.
        /// </remarks>
        /// <param name="array"> The source array.</param>
        /// <param name="length">Expected length of array.</param>
        /// <returns>
        ///     <paramref name="array" /> itself if <c>array.Length &gt;= length</c>. Otherwise a new array of same type of
        ///     <paramref name="array" /> of given <paramref name="length" />.
        /// </returns>
        ...
        /// <summary>
        ///     Does the actual work of copying to array. Subclass is recommended to override this method instead of
        ///     <see cref="CopyTo(T[], Int32)" /> method, which does all necessary parameter checking and raises proper
        ...
        /// <returns>
        ///     A new array of same runtime type as <paramref name="array" /> if <paramref name="array" /> is too small to hold all
        ///     elements and <paramref name="ensureCapacity" /> is <c>false</c>. Otherwise the
        ///     <paramref
        ///         name="array" />
        ///     instance itself.
        /// </returns>
        ...
        /// <remarks>
        ///     <para>
        ///         Attempts to <see cref="AddRange" /> of a collection to itself result in <see cref="ArgumentException" /> .
        ///         Further, the behavior of this operation is undefined if the specified collection is modified while the
        ///         operation is in progress.
        ///     </para>
        ///     <para>
        ///         This implementation iterates over the specified collection, and adds each element returned by the iterator to
        ///         this collection, in turn. An exception encountered while trying to add an element may result in
        ...
        ///     If the supplied <paramref name="collection" /> is
        ///     <see langword="null" />.
        /// </exception>
        ...
        public virtual Boolean AddRange( IEnumerable<T> collection ) {
            ...
            if ( ReferenceEquals( collection, this ) ) { throw new ArgumentException( "Cannot add to itself.", nameof( collection ) ); }

            return this.DoAddRange( collection: collection );
        }

        /// <summary>
        ///     Removes all items from the <see cref="ICollection{T}" />. This implementation always throw
        ///     <see cref="NotSupportedException" />.
        /// </summary>
        /// <exception cref="NotSupportedException">
        ///     The <see cref="ICollection{T}" /> is read-only. This implementation always
        ...
        /// <summary>
        ///     Determines whether the <see cref="ICollection{T}" /> contains a specific value. This implementation searches the
        ///     element by iterating through the enumerator returned by <see cref="GetEnumerator()" /> method.
        /// </summary>
        /// <returns>true if item is found in the <see cref="ICollection{T}" />; otherwise, false.</returns>
        /// <param name="item">The object to locate in the <see cref="ICollection{T}" />.</param>
        public virtual Boolean Contains( T item ) => this.Contains( item );

        /// <summary>
        ///     Copies the elements of the <see cref="ICollection{T}" /> to an <see cref="Array" />, starting at a particular
        ///     <see cref="Array" /> index.
        /// </summary>
        /// <remarks>
        ///     This method is intentionally sealed. Subclass should override <see cref="DoCopyTo(T[], Int32, Boolean)" />
        ///     instead.
        /// </remarks>
        /// <param name="array">
        ///     The one-dimensional <see cref="Array" /> that is the destination of the elements copied from
        ...
        /// <exception cref="ArgumentNullException">array is null.</exception>
        /// <exception cref="ArgumentException">
        ///     array is multidimensional. <br />-or- <br /> arrayIndex is equal to or greater than the length of array. <br />-or-
        ///     <br /> The number of elements in the source <see cref="ICollection{T}" /> is greater than the
        ///     available space from arrayIndex to the end of the destination array. <br />-or- <br /> Type T cannot be cast
        ...
        public void CopyTo( T[] array, Int32 arrayIndex ) {
            ...
            if ( arrayIndex < array.GetLowerBound( dimension: 0 ) ) {
                throw new ArgumentOutOfRangeException( nameof( arrayIndex ), actualValue: arrayIndex, "arrayIndex must not be less then the lower bound of the array." );
            }

            try { this.DoCopyTo( array: array, arrayIndex: arrayIndex, ensureCapacity: false ); }
            catch ( IndexOutOfRangeException e ) { throw new ArgumentException( "array is too small to fit the collection.", nameof( array ), innerException: e ); }
        }
        ...
        /// </summary>
        /// <returns>
        ///     true if item was successfully removed from the <see cref="ICollection{T}" />; otherwise, false. This method also
        ///     returns false if item is not found in the original <see cref="ICollection{T}" /> .
        /// </returns>
        ...
        /// <summary>
        ///     Returns an array containing all of the elements in this collection, in proper sequence; the runtime type of the
        ///     returned array is that of the specified array. If the collection fits in the specified array, it
        ///     is returned therein. Otherwise, a new array is allocated with the runtime type of the specified array and the size
        ///     of this collection.
        /// </summary>
        /// <remarks>
        ///     <para>
        ///         Like the <see cref="ToArray()" /> method, this method acts as bridge between array-based and collection-based
        ///         APIs. Further, this method allows precise control over the runtime type of the output array, and
        ///         may, under certain circumstances, be used to save allocation costs.
        ///     </para>
        ///     <para>
        ///         Suppose <i>x</i> is a collection known to contain only strings. The following code can be used to dump the
        ///         collection into a newly allocated array of <see cref="String" /> s:
        ///         <code language="c#">
        ///  String[] y = (String[])
        ...
        ///     </para>
        ///     <para>
        ///         Note that <i>ToArray(new T[0])</i> is identical in function to <see cref="AbstractCollection{T}.ToArray()" />
        ///         .
        ///     </para>
        /// </remarks>
        ...
        ///         words, this method must allocate a new array). The caller is thus free to modify the returned array.
        ///     </para>
        ...
        ///     array is multidimensional.-or- index is equal to or greater than the length of array.-or- The number of elements in
        ///     the source <see cref="T:System.Collections.ICollection" /> is greater than the available space
        ...
        void ICollection.CopyTo( Array array, Int32 index ) {
            ...
            if ( index < array.GetLowerBound( dimension: 0 ) ) { throw new ArgumentOutOfRangeException( nameof( index ), actualValue: index, "index must not be less then lower bound of the array" ); }

            try { this.CopyTo( array: array, index: index ); }
            catch ( RankException re ) { throw new ArgumentException( "array must not be multi-dimensional.", nameof( array ), innerException: re ); }
            catch ( IndexOutOfRangeException e ) { throw new ArgumentException( "array is too small to fit the collection.", nameof( array ), innerException: e ); }
        }
