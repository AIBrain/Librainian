...
// 
// File "BenchmarkTests.cs" last touched on 2022-01-19 at 5:03 AM by Protiguous.

namespace LibrainianUnitTests.Measurement;
...
public static class BenchmarkTests {
	...
	public static void TestBenchmarking_MethodA() {
		static void A() => Thread.Sleep( Jitter.Half() );
		static void B() => Thread.Sleep( Jitter.Twice() );

		var result = Benchmark.WhichIsFaster( A, B, Milliseconds.ThreeHundredThirtyThree );
		var _ = result.Should()!.Be( Benchmark.AorB.MethodA );
	}

	[Test]
	public static void TestBenchmarking_MethodB() {
		static void A() => Thread.Sleep( Jitter.Twice() );
		static void B() => Thread.Sleep( Jitter.Half() );
		var result = Benchmark.WhichIsFaster( A, B, Milliseconds.ThreeHundredThirtyThree );
