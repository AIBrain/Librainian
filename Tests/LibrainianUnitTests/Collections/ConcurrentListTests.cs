...
// 
// File "ConcurrentListTests.cs" last touched on 2021-10-25 at 7:57 AM by Protiguous.
...
public static class ConcurrentListTests {
	...
	private static Int32 AddManyProducts( ICollection<String> list ) {
		Parallel.For( 0, Threads, x => list.Add( $"Item {x}" ) );
    ...
    */
    ...
    public static async Task TestAFew() {
        ...
        TestContext.WriteLine( $"Creating {create} workers..." );
        ...
        TestContext.WriteLine( $"Waiting for {workers.Count} workers to complete..." );

        while ( workers.Any( worker => worker?.IsDone() == false ) ) {
            await Task.Delay( 100, token ).ConfigureAwait( false );
        }

        TestContext.WriteLine( $"We now have {numbers.Count} numbers." );
