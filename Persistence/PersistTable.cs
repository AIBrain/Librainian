    ...
    public sealed class PersistTable<TKey, TValue> : ABetterClassDispose, IDictionary<TKey, TValue> where TKey : IComparable<TKey> {
        ...
        public PersistTable( [CanBeNull] Folder folder, Boolean testForReadWriteAccess = false ) {
            try {
                ...
                if ( testForReadWriteAccess && !this.TestForReadWriteAccess() ) {
                    throw new IOException( message: $"Read/write permissions denied in folder {this.Folder.FullName}." );
                }
            }
        ...
        public TValue this[[NotNull] TKey key] {
            ...
            get {
                ...
                if ( !this.Dictionary.TryGetValue( key, value: out var storedValue ) ) {
                    return default;
                }
        ...
        /// </summary>
        /// <returns></returns>
        public IEnumerable<KeyValuePair<TKey, TValue>> Items() => this.Dictionary.Select( selector: pair => new KeyValuePair<TKey, TValue>( pair.Key, value: pair.Value.FromCompressedBase64().FromJSON<TValue>() ) );
