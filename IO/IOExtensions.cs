...
// 
// "Librainian/IOExtensions.cs" was last cleaned by Rick on 2014/12/09 at 5:56 AM
...
//check for a double extension (image.jpg.tif), remove the 'fake' (.jpg) extension
		...
		/// <summary>
		/// Given the <paramref name="path"/> and <paramref name="searchPattern"/> pick any one file
		/// and return the <see cref="FileSystemInfo.FullName"/> .
		/// </summary>
		...
		public static Boolean SameContent( [CanBeNull] this Document left, [CanBeNull] Document right ) {
			...
			return linfo.Exists && rinfo.Exists && linfo.SameContent( rinfo );
		}
		...
		public static Boolean SameContent( [CanBeNull] this FileInfo left, [CanBeNull] FileInfo right ) {
			...
			return left.Exists && right.Exists && left.Length == right.Length && left.AsByteArray().SequenceEqual( right.AsByteArray() );
		}
