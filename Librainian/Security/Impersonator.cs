...
//
// ***  Project "Librainian"  ***
// File "Impersonator.cs" was last formatted by Protiguous on 2018/06/26 at 1:41 AM.
		...
		private void ImpersonateValidUser( String userName, String domain, String password ) {
			...
			try {
				if ( !NativeMethods.RevertToSelf() ) {
					...
					if ( NativeMethods.LogonUser( lpszUserName: userName, lpszDomain: domain, lpszPassword: password, dwLogonType: Logon32LogonInteractive, dwLogonProvider: Logon32ProviderDefault,
						     phToken: ref token ) == 0 ) {
						throw new Win32Exception( error: Marshal.GetLastWin32Error() );
					}
					else {
