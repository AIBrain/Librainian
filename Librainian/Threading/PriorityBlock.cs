...
//
// This source code contained in "PriorityBlock.cs" belongs to Protiguous@Protiguous.com
...
//
// ***  Project "Librainian"  ***
// File "PriorityBlock.cs" was last formatted by Protiguous on 2018/06/26 at 1:43 AM.
	...
	public class PriorityBlock {
		...
		public async Task AddJobs( [NotNull] IEnumerable<OneJob> jobs, CancellationToken token ) {
			...
			foreach ( var job in enumerable.OrderByDescending( job => job.Priority ) ) {
				await this.Input.TryPost( job, token );
			}
		}
	}
}