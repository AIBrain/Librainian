...
//
// File "UsableSemaphoreThrottle.cs" last formatted on 2021-11-30 at 7:23 PM by Protiguous.
...
public class UsableSemaphoreThrottle : IUsableSemaphore {
	...
	public UsableSemaphoreThrottle( IThrottle throttle, IUsableSemaphore semaphore ) {
		this._throttle = throttle ?? throw new NullException( nameof( throttle ) );
		this._semaphore = semaphore ?? throw new NullException( nameof( semaphore ) );
	}
	...
	public async Task<IUsableSemaphoreWrapper> WaitAsync( CancellationToken cancellationToken ) {
		...
		try {
			...
			await this._throttle.WaitAsync( cancellationToken ).ConfigureAwait( false );
