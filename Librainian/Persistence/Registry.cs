...
namespace Librainian.Persistence {
	...
	public static class Registry {
		...
		/// </summary>
		[NotNull]
		internal static RegistryKey OurApplication { get; }

		/// <summary>
		///     Registry key for the product's company.
		/// </summary>
		[NotNull]
		internal static RegistryKey OurCompany { get; }

		/// <summary>
		///     Registry key for the current user;
		/// </summary>
		[NotNull]
		internal static RegistryKey OurUser { get; } = Microsoft.Win32.Registry.CurrentUser;

		/// <summary>
		///     Registry key for the user's software.
		/// </summary>
		[NotNull]
		internal static RegistryKey Software { get; }

		static Registry() {
			if ( OurUser is null ) {
				throw new ArgumentEmptyException( $"Registry folder {nameof( Microsoft.Win32.Registry.CurrentUser )} is null!" );
			}

			Software = OurUser.CreateSubKey( nameof( Software ), true );

			if ( Software is null ) {
				throw new ArgumentEmptyException( $"Application {nameof( Registry )} folder {nameof( Software )} is null!" );
			}

			OurCompany = Software.CreateSubKey( Application.CompanyName.Replace( "&", String.Empty ), true );

			if ( OurCompany is null ) {
				throw new ArgumentEmptyException( $"Application {nameof( Registry )} folder {nameof( Application.CompanyName )} is null!" );
			}

			OurApplication = OurCompany.CreateSubKey( Application.ProductName, true );

			if ( OurApplication is null ) {
				throw new ArgumentEmptyException( $"Application {nameof( Registry )} folder {nameof( Application.ProductName )} is null!" );
			}
		}
		...
		/// <returns></returns>
		public static void Set( TrimmedString folder, TrimmedString key, [CanBeNull] Object value, RegistryValueKind kind ) {
			if ( folder.IsEmpty() ) {
