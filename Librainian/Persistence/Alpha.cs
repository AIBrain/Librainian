...
//
// ***  Project "Librainian"  ***
// File "Alpha.cs" was last formatted by Protiguous on 2018/06/26 at 1:37 AM.
...
*/
		...
		public static class Storage {
			...
			public static Folder RootPath { get; } =
				new Folder( Path.Combine( Environment.GetFolderPath( Environment.SpecialFolder.CommonApplicationData ), Path.Combine( nameof( Storage ), nameof( Root ) ) ) );
			...
			private static Boolean DiscoverLocalResources() {
				try {
					...
					LocalResourceDiscoveryTime.Finished = DateTime.UtcNow;
				}
			...
			private static Boolean DiscoverRemoteResources() {
				try {
					...
					RemoteResourceDiscoveryTime.Finished = DateTime.UtcNow;
				}
			...
			public static async Task Initialize() {
				try {
					...
					InitializeTime.Finished = DateTime.UtcNow;
				}
			}
