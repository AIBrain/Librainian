...
namespace Librainian.Persistence.InIFiles {
    ...
    public class Ini : IIni, IEquatable<Ini> {
        ...
        public Boolean Clear() {
            Parallel.ForEach( source: this.Data.Keys.AsParallel(), body: section => this.TryRemove( section: section ) );
        ...
        [NotNull]
        public Task<Boolean> ReloadAsync( CancellationToken token = default ) {
            ...
            return Task.Run( function: () => {
                ...
                try {
                    ...
                    var result = Parallel.ForEach( source: data.Keys.AsParallel(), parallelOptions: CPU.AllCPUExceptOne,
                        body: section => Parallel.ForEach( source: data[ section ].Keys.AsParallel(), parallelOptions: CPU.AllCPUExceptOne,
        ...
        /// <returns></returns>
        [NotNull]
        public Task<Boolean> WriteAsync( CancellationToken cancellationToken = default ) =>
            Task.Run( function: () => {
                if ( this.Document == null ) {
                    return false;
                }

                if ( this.Document.Exists() == true ) {
                    this.Document.Delete();
                }

                return this.Data.TrySave( document: this.Document, overwrite: true, formatting: Formatting.Indented );
        ...
        public Ini( [NotNull] Document document, CancellationTokenSource cancellationSource = null ) {
            ...
            this.ReloadAsync().Wait( cancellationToken: cancellationSource.Token );
        }
