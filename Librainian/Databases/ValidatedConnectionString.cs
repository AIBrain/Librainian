...
//
// File "ValidatedConnectionString.cs" last formatted on 2021-11-30 at 7:16 PM by Protiguous.
...
public record ValidatedConnectionString : IValidatedConnectionString {
	...
	/// <summary>
	/// Uses the <paramref name="secretRevealer" /> to obtain a database connection string and validated it through a <see
	...
	public ValidatedConnectionString( ISecretRevealer secretRevealer, IApplicationSetting applicationSetting, IAppCache cache ) {
		if ( secretRevealer == null ) {
			throw new ArgumentNullException( nameof( secretRevealer ) );
		}
		...
		var key = Common.ToKey( nameof( this.Value ), nameof( DatabaseServer ), nameof( ValidatedConnectionString ) );
