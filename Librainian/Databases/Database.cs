...
namespace Librainian.Databases {
    ...
    public sealed class Database : ABetterClassDispose, IDatabase {
        ...
        public static async Task<ConcurrentDictionary<String, ServiceControllerStatus>> FindAndStartSqlBrowserServices( [NotNull] IEnumerable<String> activeMachines,
            ...
            tasks.AddRange( machines.Select( machine => Task.Run( () => method( machine: machine ), new CancellationTokenSource( timeout ).Token ) ) );

            await Task.WhenAny( tasks ).ConfigureAwait( false );

            return status;

            void method( String machine ) {
                try {
        ...
        [NotNull]
        public static Task StartAnySQLBrowsers( TimeSpan searchTimeout ) =>
