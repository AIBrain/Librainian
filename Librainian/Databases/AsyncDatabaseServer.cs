...
// 
// Project: "Librainian", "AsyncDatabaseServer.cs" was last formatted by Protiguous on 2019/11/04 at 4:35 AM.
    ...
    public class AsyncDatabaseServer : ABetterClassDispose, IDatabase {
        ...
        public async Task<DataTableReader> ExecuteReaderAsyncDataReader( [NotNull] String query, CommandType commandType, [CanBeNull] params SqlParameter[] parameters ) {
            ...
            try {
                ...
                return readerAsync.ToDataTable().CreateDataReader();
            }
            catch ( SqlException exception ) {
                exception.Log( Rebuild( query, parameters ) );
            }
        ...
        public (Status status, T result) ExecuteScalar<T>( String query, CommandType commandType, [CanBeNull] params SqlParameter[] parameters ) {
            ...
            try {
                ...
                return command.ExecuteScalar().ConvertScalar<T>();
            }
            catch ( SqlException exception ) {
                exception.Log( Rebuild( query, parameters ) );
            }
        ...
        [NotNull]
        [DebuggerStepThrough]
        public static IEnumerable<(String, ServiceControllerStatus)> StartAnySQLBrowsers( TimeSpan timeout ) {

            "Searching for any database servers...".Log();

            var machines = new ConcurrentHashset<String>( ) {
                "Thor",Dns.GetHostName(),"127.0.0.1", "Thor2" //my development servers' names. Feel free to add your server.
            };

            foreach ( var machine in machines.Where( s => !String.IsNullOrWhiteSpace( s ) ) ) {

                $"Searching for database servers on {machine}...".Log();
                ...
                if ( service.Status.In( ServiceControllerStatus.StartPending, ServiceControllerStatus.Running ) ) {
                    yield return (machine, service.Status);
                }
            }
        }
        ...
        public static IEnumerable<SqlServer> FindUsableServers( TimeSpan timeout, [NotNull] params Credentials[] credentials ) {
            ...
            try {
                ...
                var servers = credentials
                    .Where( c => c != default )
                    .SelectMany( credential => LookForAnyDatabaseServerWithThisCredential( credential, timeout ) )
                    .Select( builder => builder?.TryGetResponse( cts.Token ) );

                return servers.Where( builder => builder != null && builder.Status == Status.Success );
            }
        ...
        public DataTableReader QueryAdHoc( [NotNull] String sql ) {
            ...
            return command.ExecuteReader().ToDataTable().CreateDataReader();
        }
        ...
        public async Task<DataTableReader> QueryAdHocAsync( [NotNull] String sql ) {
            ...
            var result = await command.ExecuteReaderAsync( this.Token ).ConfigureAwait( false );

            return result.ToDataTable().CreateDataReader();
        }
