...
//
// Project: "Librainian", File: "TimeSpanRange.cs" was last formatted by Protiguous on 2020/03/16 at 2:57 PM.
...
namespace Librainian.Measurement.Time {
    ...
    public struct TimeSpanRange {
        ...
        public IEnumerable<Days> AllDays() {
            return this.Min.TotalDays.To( end: this.Max.TotalDays, step: 1 ).Select( selector: second => Days.One );
        }

        [NotNull]
        public IEnumerable<Hours> AllHours() {
            return this.Min.TotalHours.To( end: this.Max.TotalHours, step: 1 ).Select( selector: second => Hours.One );
        }

        [NotNull]
        public IEnumerable<Milliseconds> AllMilliseconds() {
            return this.Min.TotalMilliseconds.To( end: this.Max.TotalMilliseconds, step: 1 ).Select( selector: millsecond => Milliseconds.One );
        }

        [NotNull]
        public IEnumerable<Minutes> AllMinutes() {
            return this.Min.TotalMinutes.To( end: this.Max.TotalMinutes, step: 1 ).Select( selector: minutes => Minutes.One );
        }

        [NotNull]
        public IEnumerable<Seconds> AllSeconds() {
            return this.Min.TotalSeconds.To( end: this.Max.TotalSeconds, step: 1 ).Select( selector: second => Seconds.One );
        }
