...
// 
// File "PriCommon.cs" last formatted on 2021-12-02 at 7:27 AM by Protiguous.
...
public static class PriCommon {
	...
	public static Exception GetExceptionFromWin32Error( PriNativeMethods.ERROR errorCode, String parameterName ) {
		...
		return errorCode switch {
			...
			PriNativeMethods.ERROR.ERROR_INVALID_NAME => new ArgumentNullException( parameterName, message ),
	...
	*/
	...
	/// <summary>Returns the trimmed string or throws <see cref="NullException" /> if null, whitespace, or empty.</summary>
	/// <param name="path"></param>
	/// <exception cref="NullException">Gets thrown if the <paramref name="path" /> is null, whitespace, or empty.</exception>
	...
	public static void ThrowIOError( PriNativeMethods.ERROR errorCode, String maybeFullPath ) {
		if ( String.IsNullOrWhiteSpace( maybeFullPath ) ) {
			throw new NullException( nameof( maybeFullPath ) );
		}
		...
		switch ( errorCode ) {
			...
			case PriNativeMethods.ERROR.ERROR_PATH_NOT_FOUND:
				...
				if ( str.Length == 0 ) {
					...
					throw new DirectoryNotFoundException( $"Directory {str.SmartQuote()} not found" );
				}
			...
			case PriNativeMethods.ERROR.ERROR_ACCESS_DENIED:
				...
				if ( str.Length == 0 ) {
					...
					throw new UnauthorizedAccessException( $"Access denied accessing {str.SmartQuote()}" );
				}
