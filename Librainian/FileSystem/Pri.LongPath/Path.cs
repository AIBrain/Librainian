...
public static class Path {
	...
	public static String CombinePaths( params String[] paths ) {
		if ( paths == null ) {
			throw new NullException( nameof( paths ) );
		}
		...
		switch ( paths.Length ) {
			...
			case 1: {
				...
				//if ( z == null ) {
				//throw new NullException( "Value cannot be null or whitespace." );
				//}
	...
	/// <param name="path"></param>
	/// <exception cref="InvalidOperationException">Thrown if any invalid chars found in path.</exception>
	[Pure]
	...
	public static void ThrowIfInvalidPathChars( ref String path ) {
		var list = new List<Char>( path.Where( c => InvalidPathChars.Contains( c ) ).Distinct() );
		if ( list.Any() ) {
			throw new InvalidOperationException( $"Invalid characters {list.ToStrings().DoubleQuote()} in {nameof( path )}" );
		}
	}
