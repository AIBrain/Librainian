...
//
// File "MkModel.cs" last formatted on 2021-11-30 at 7:20 PM by Protiguous.
...
public class MkModel {
	...
	public Task<String> GenerateRandomCorpus( Int32 numberOfWords ) =>
		Task.Run( () => {
			...
			foreach ( var _ in 0.To( numberOfWords ) ) {
				var randomChain = this.Nexts( word ).Where( w => !String.IsNullOrEmpty( w ) ).OrderBy( _ => Randem.Next() );

				foreach ( var w in randomChain ) {
					corpus.Append( w ).Append( ParsingConstants.Chars.Space );
	...
	public IEnumerable<Word> Nexts( Word? word ) {
		if ( word is not null ) {
			if ( this.MarkovChains.TryGetValue( word, out var corpus ) ) {
				return corpus;
			}
		}
	...
	public async Task Train( String? corpus, Int32 level = 3 ) =>
		await Task.Run( () => {
			var words = corpus.ToWords().ToListTrimExcess();

			foreach ( (var word, var index) in words.Select( ( word, i ) => (word, i) ) ) {
				var chain = words.Skip( index + 1 ).Take( level );
				this.MarkovChains.TryAdd( word, chain.ToListTrimExcess() );

				//TODO What will this do with duplicate Words in a sentence?
			}
		} )
