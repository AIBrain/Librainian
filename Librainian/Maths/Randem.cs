...
public static class Randem {
	...
	*/
	...
	public static String RandomSentence( Byte wordCount = 7 ) {
		...
		var high = wordCount + low;

		for ( var i = 0; i < Next( low, high + 1 ); i++ ) {
			list.Add( RandomWord() );
		}
	...
	///     <paramref
	///         name="lowerCase" />
	///     is true. Default <paramref name="upperCase" /> is false. Default <paramref name="numbers" /> is
	...
	public static String RandomString( Int32 length = 16, Boolean lowerCase = true, Boolean upperCase = false, Boolean numbers = false, Boolean symbols = false ) {
		...
		return new String( Enumerable.Range( 0, length ).Select( _ => charPool[ 0.Next( poolLength ) ] ).ToArray() );
	}
