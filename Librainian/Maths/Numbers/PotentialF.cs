...
//
// This source code contained in "PotentialF.cs" belongs to Protiguous@Protiguous.com unless otherwise specified or the original license has been overwritten
...
//
// Project: "Librainian", File: "PotentialF.cs" was last formatted by Protiguous on 2020/03/16 at 2:56 PM.
...
namespace Librainian.Maths.Numbers {
    ...
    public sealed class PotentialF {
        ...
        /// </remarks>
        public Single Value {
            get => Thread.VolatileRead( address: ref this._value );

            private set => Thread.VolatileWrite( address: ref this._value, value: value >= MaxValue ? MaxValue : value <= MinValue ? MinValue : value );
        }
        ...
        public PotentialF( Boolean randomValue ) {
            if ( randomValue ) {
                this.Value = Randem.NextFloat( min: MinValue, max: MaxValue );
            }
        }
        ...
        public PotentialF( Single min, Single max ) : this( initialValue: Randem.NextFloat( min: min, max: max ) ) { }

        public static implicit operator Single( [NotNull] PotentialF special ) => special.Value;

        [NotNull]
        public static PotentialF Parse( [NotNull] String value ) => new PotentialF( initialValue: Single.Parse( s: value ) );
