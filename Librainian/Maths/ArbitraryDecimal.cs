...
// 
// This source code contained in "ArbitraryDecimal.cs" belongs to Rick@AIBrain.org and
...
// 
// ***  Project "Librainian"  ***
// File "ArbitraryDecimal.cs" was last formatted by Protiguous on 2018/06/04 at 4:03 PM.
	...
	public class ArbitraryDecimal {
		...
		public void Normalize() {
			...
			for ( first = 0; first < this._digits.Length; first++ ) {
				if ( this._digits[ first ] != 0 ) { break; }
			}

			Int32 last;

			for ( last = this._digits.Length - 1; last >= 0; last-- ) {
				if ( this._digits[ last ] != 0 ) { break; }
			}

			if ( first == 0 && last == this._digits.Length - 1 ) { return; }

			var tmp = new Byte[ last - first + 1 ];

			for ( var i = 0; i < tmp.Length; i++ ) { tmp[ i ] = this._digits[ i + first ]; }
		...
		public override String ToString() {
			...
			for ( var i = 0; i < this._digits.Length; i++ ) { digitString[ i ] = ( Char ) ( this._digits[ i ] + '0' ); }

			// Simplest case - nothing after the System.Decimal point, and last real digit is
			// non-zero, eg value=35
			if ( this._decimalPoint == 0 ) { return new String( digitString ); }

			// Fairly simple case - nothing after the System.Decimal point, but some 0s to add,
			// eg value=350
			if ( this._decimalPoint < 0 ) { return $"{new String( digitString )}{new String( '0', -this._decimalPoint )}"; }

			// Nothing before the System.Decimal point, eg 0.035
			if ( this._decimalPoint >= digitString.Length ) { return $"0.{new String( '0', this._decimalPoint - digitString.Length )}{new String( digitString )}"; }
