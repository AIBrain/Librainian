    ...
    public class Volume : Device {
        ...
        public IEnumerable<Int32> GetDiskNumbers() {
            ...
            if ( this.GetLogicalDrive() != null ) {
                ...
                var hFile = NativeMethods.CreateFile( $@"\\.\{this.GetLogicalDrive()}", 0, FileShare.ReadWrite,
                ...
                try {
                    if ( !NativeMethods.DeviceIoControl( hFile.DangerousGetHandle(), NativeMethods.IOCTL_VOLUME_GET_VOLUME_DISK_EXTENTS,
                        IntPtr.Zero, 0, buffer, size, out bytesReturned,
                        IntPtr.Zero ) ) {
