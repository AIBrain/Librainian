...
//
// ***  Project "Librainian"  ***
// File "DurableDatabase.cs" was last formatted by Protiguous on 2018/06/26 at 12:59 AM.
		...
		public async Task<DataTable> ExecuteReaderAsyncDataTable( String query, CommandType commandType, [CanBeNull] params SqlParameter[] parameters ) {
			...
			try {
				using ( var command = new SqlCommand( query, this.OpenConnection() ) {
					...
					using ( var reader = await command.ExecuteReaderAsync( this.CancelConnection.Token ).NoUI() ) {
						table.Load( reader );
					}
		...
		public async Task<TResult> ExecuteScalarAsync<TResult>( String query, CommandType commandType, [CanBeNull] params SqlParameter[] parameters ) {
			...
			try {
				using ( var command = new SqlCommand( query, this.OpenConnection() ) {
					...
					try {
						scalar = await command.ExecuteScalarAsync().NoUI();
					}
					catch ( SqlException exception ) {
