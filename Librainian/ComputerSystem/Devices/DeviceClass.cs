...
namespace Librainian.ComputerSystem.Devices {
	...
	public abstract class DeviceClass : ABetterClassDispose {
		...
		internal NativeMethods.SP_DEVINFO_DATA GetInfo( Int32 dnDevInst ) {
			...
			if ( !NativeMethods.SetupDiOpenDeviceInfo( this._deviceInfoSet, sb.ToString(), IntPtr.Zero, 0,
				devData ) ) {
		...
		internal String? GetProperty( NativeMethods.SP_DEVINFO_DATA devData, UInt32 property, [CanBeNull] String defaultValue ) {
			...
			if ( !NativeMethods.SetupDiGetDeviceRegistryProperty( this._deviceInfoSet, ref devData, property,
				out var propertyRegDataType, propertyBuffer, propertyBufferSize,
		...
		public IEnumerable<Device> GetDevices() {
			...
			while ( true ) {
				...
				if ( !NativeMethods.SetupDiEnumDeviceInterfaces( this._deviceInfoSet, default, ref this._classGuid,
					index, interfaceData ) ) {
					var error = Marshal.GetLastWin32Error();
				...
				if ( !NativeMethods.SetupDiGetDeviceInterfaceDetail( this._deviceInfoSet, interfaceData,
					IntPtr.Zero, 0, ref size, devData ) ) {
					var error = Marshal.GetLastWin32Error();
				...
				if ( !NativeMethods.SetupDiGetDeviceInterfaceDetail( this._deviceInfoSet, interfaceData, buffer,
					size, ref size, devData ) ) {
					try {
						throw new Win32Exception( Marshal.GetLastWin32Error() );
					}
					finally {
				...
				if ( this._classGuid.Equals( new Guid( NativeMethods.GUID_DEVINTERFACE_DISK ) ) ) {
					...
					// Find disks
					var hFile = NativeMethods.CreateFile( devicePath, 0, FileShare.ReadWrite, IntPtr.Zero,
					...
					try {
						if ( !NativeMethods.DeviceIoControl( hFile.DangerousGetHandle(), NativeMethods.IOCTL_STORAGE_GET_DEVICE_NUMBER,
							IntPtr.Zero, 0, numBuffer, numBufSize, out bytesReturned,
							IntPtr.Zero ) ) {
					...
					if ( bytesReturned > 0 ) {
						disknum = ( NativeMethods.STORAGE_DEVICE_NUMBER )Marshal.PtrToStructure( numBuffer,
							typeof( NativeMethods.STORAGE_DEVICE_NUMBER ) );
					}
					else {
