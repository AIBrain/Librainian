...
//
// Project: "Librainian", "Unique.cs" was last formatted by Protiguous on 2018/11/19 at 7:48 PM.
	...
	public class Unique : IEquatable<Unique> {
		...
		public async Task<Int64> Length( CancellationToken token, TimeSpan timeout ) {
			try {
				using ( var client = new WebClient().Add( timeout, token ) ) {
					try {
						await client.OpenReadTaskAsync( this.ToUri() ).ConfigureAwait( false );
		...
		public DirectoryInfo ToDirectoryInfo() {
			try {
				if ( this.U.StartsWith( Protocols.File, StringComparison.Ordinal ) ) {
		...
		///         query strings.
		///     </para>
		/// <para>Caches the result, because <see cref="ToUri"/> is called in many other places.</para>
		/// <para>This should NEVER fail, because only valid Uri have been parsed in the ctor.</para>
		/// </summary>
		/// <returns></returns>
		[NotNull]
		public Uri ToUri() {
			if ( this._internalUri == null ) {
				Uri.TryCreate( this.U, UriKind.Absolute, out this._internalUri );
			}
