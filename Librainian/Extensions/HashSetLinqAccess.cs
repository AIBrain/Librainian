...
//
// File "HashSetLinqAccess.cs" last formatted on 2021-11-30 at 7:17 PM by Protiguous.
...
/// <summary>
/// This extension method class will add a ToHashSet <typeparamref name="&gt;" /> in exactly the same way it is provided by the
/// others: ToList(), ToArray(), ToDictionary().. Now ToHashSet() is available. UPDATE: These might be available in the newer Libraries.
/// </summary>
...
public static class HashSetLinqAccess {
	...
	public static HashSet<T> AddRange<T>( this HashSet<T> hashSet, IEnumerable<T> range ) {
		if ( Equals( hashSet, null ) ) {
			throw new NullException( nameof( hashSet ) );
		}
	...
	public static HashSet<T> ToHashSet<T>( this IEnumerable<T> fromEnumerable, IEqualityComparer<T>? comparer ) {
		if ( fromEnumerable is null ) {
			throw new NullException( nameof( fromEnumerable ) );
		}
