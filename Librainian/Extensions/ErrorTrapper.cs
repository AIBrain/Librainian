...
//
// File "ErrorTrapper.cs" last formatted on 2022-12-22 at 5:15 PM by Protiguous.
...
public static class ErrorTrapper {
	...
	/// <param name="actions"></param>
	[DebuggerStepThrough]
	public static R? Trap<T, R>( this Func<T?, R>? func, T? argument, out Exception? exception, Action? final = default( Action? ), params Action[]? actions ) {
		if ( func is null ) {
			if ( Debugger.IsAttached ) {
				throw new ArgumentEmptyException( nameof( func ) );
			}
