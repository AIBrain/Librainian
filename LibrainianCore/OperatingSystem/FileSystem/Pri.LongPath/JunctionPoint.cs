...
// 
// Project: "LibrainianCore", File: "JunctionPoint.cs" was last formatted by Protiguous on 2020/03/16 at 3:09 PM.
...
namespace Librainian.OperatingSystem.FileSystem.Pri.LongPath {
    ...
    public static class JunctionPoint {
        ...
        private static String? InternalGetTarget( SafeFileHandle handle ) {
            var outBufferSize = Marshal.SizeOf( t: typeof( REPARSE_DATA_BUFFER ) );
            var outBuffer = Marshal.AllocHGlobal( cb: outBufferSize );

            try {
                var result = DeviceIoControl( hDevice: handle.DangerousGetHandle(), dwIoControlCode: FSCTL_GET_REPARSE_POINT, InBuffer: IntPtr.Zero, nInBufferSize: 0,
                    OutBuffer: outBuffer, nOutBufferSize: outBufferSize, pBytesReturned: out var bytesReturned, lpOverlapped: IntPtr.Zero );
                ...
                var reparseDataBuffer = ( REPARSE_DATA_BUFFER ) Marshal.PtrToStructure( ptr: outBuffer, structureType: typeof( REPARSE_DATA_BUFFER ) );
        ...
        private static SafeFileHandle OpenReparsePoint( [NotNull] String reparsePoint, EFileAccess accessMode ) {
            var reparsePointHandle = new SafeFileHandle(
                preexistingHandle: CreateFile( lpFileName: reparsePoint, dwDesiredAccess: accessMode, dwShareMode: EFileShare.Read | EFileShare.Write | EFileShare.Delete,
                    lpSecurityAttributes: IntPtr.Zero, dwCreationDisposition: ECreationDisposition.OpenExisting,
                    dwFlagsAndAttributes: EFileAttributes.BackupSemantics | EFileAttributes.OpenReparsePoint, hTemplateFile: IntPtr.Zero ), ownsHandle: true );
        ...
        private static void ThrowLastWin32Error( [NotNull] String message ) =>
        ...
        public static void Create( [NotNull] String junctionPoint, [NotNull] String targetDir, Boolean overwrite ) {
            ...
            using ( var handle = OpenReparsePoint( reparsePoint: junctionPoint, accessMode: EFileAccess.GenericWrite ) ) {
                var targetDirBytes = Encoding.Unicode.GetBytes( s: NonInterpretedPathPrefix + targetDir.GetFullPath() );
                ...
                Array.Copy( sourceArray: targetDirBytes, destinationArray: reparseDataBuffer.PathBuffer, length: targetDirBytes.Length );
                ...
                try {
                    ...
                    var result = DeviceIoControl( hDevice: handle.DangerousGetHandle(), dwIoControlCode: FSCTL_SET_REPARSE_POINT, InBuffer: inBuffer,
        ...
        public static void Delete( [NotNull] String junctionPoint ) {
            ...
            using ( var handle = OpenReparsePoint( reparsePoint: junctionPoint, accessMode: EFileAccess.GenericWrite ) ) {
                ...
                try {
                    ...
                    var result = DeviceIoControl( hDevice: handle.DangerousGetHandle(), dwIoControlCode: FSCTL_DELETE_REPARSE_POINT, InBuffer: inBuffer, nInBufferSize: 8,
                        OutBuffer: IntPtr.Zero, nOutBufferSize: 0, pBytesReturned: out var bytesReturned, lpOverlapped: IntPtr.Zero );
        ...
        public static Boolean Exists( [NotNull] String path ) {
            ...
            using ( var handle = OpenReparsePoint( reparsePoint: path.ThrowIfBlank(), accessMode: EFileAccess.GenericRead ) ) {
        ...
        public static String GetTarget( [NotNull] String junctionPoint ) {
            using ( var handle = OpenReparsePoint( reparsePoint: junctionPoint, accessMode: EFileAccess.GenericRead ) ) {
                var target = InternalGetTarget( handle: handle );
