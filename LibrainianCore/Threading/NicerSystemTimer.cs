...
// 
// Project: "LibrainianCore", File: "NicerSystemTimer.cs" was last formatted by Protiguous on 2020/03/16 at 3:12 PM.
...
namespace Librainian.Threading {
    ...
    public class NicerSystemTimer : IDisposable {
        ...
        public NicerSystemTimer( [CanBeNull] Action action, Boolean repeat, Double? milliseconds = null ) {
            ...
            this.access = new ReaderWriterLockSlim( recursionPolicy: LockRecursionPolicy.SupportsRecursion );

            this.Timer = new Timer {
                AutoReset = false, Interval = milliseconds.GetValueOrDefault( defaultValue: 1 )
            };

            this.Timer.Elapsed += ( sender, args ) => {

                try {
                    if ( this.access.TryEnterReadLock( millisecondsTimeout: 0 ) ) {
                        this.Timer.Stop();
                        action.Invoke();
                    }
