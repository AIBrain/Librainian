...
// 
// This source code contained in "Job.cs" belongs to Protiguous@Protiguous.com unless otherwise specified or the original license has been overwritten
...
// 
// Project: "LibrainianCore", File: "Job.cs" was last formatted by Protiguous on 2020/03/16 at 3:12 PM.
...
namespace Librainian.Threading {
    ...
    public class Job<T> {
        ...
        private Job( TimeSpan maxRuntime ) {
            ...
            this.CTS = new CancellationTokenSource( delay: this.MaxRunningTime );
        }

        public Job( [NotNull] Func<T> func, TimeSpan maxRuntime ) : this( maxRuntime: maxRuntime ) {
            if ( func is null ) {
            ...
            this.TheTask = new Task( action: () => result = func.Trap(), cancellationToken: this.CTS.Token, creationOptions: TaskCreationOptions.PreferFairness ).ContinueWith(
                continuationAction: task => this.Done( result: result ),
                continuationOptions: TaskContinuationOptions.AttachedToParent | TaskContinuationOptions.ExecuteSynchronously );
        }

        public Job( [NotNull] Action action, TimeSpan maxRuntime ) : this( maxRuntime: maxRuntime ) {
            if ( action is null ) {
                throw new ArgumentNullException( paramName: nameof( action ) );
            }

            this.TheTask = new Task( action: () => action.Trap(), cancellationToken: this.CTS.Token, creationOptions: TaskCreationOptions.PreferFairness ).ContinueWith(
                continuationAction: task => this.Done( result: default ),
                continuationOptions: TaskContinuationOptions.AttachedToParent | TaskContinuationOptions.ExecuteSynchronously );
        }
        ...
        public void AddRunningTime( TimeSpan timeSpan ) {
            if ( !this.IsDone() ) {
                ...
                this.CTS = new CancellationTokenSource( delay: this.MaxRunningTime - this.Elapsed() );
            }
        }
        ...
        public async Task Task() {
            try {
                await this.TheTask.ConfigureAwait( continueOnCapturedContext: false );
            }
