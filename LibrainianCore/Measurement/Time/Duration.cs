...
// 
// This source code contained in "Duration.cs" belongs to Protiguous@Protiguous.com unless otherwise specified or the original license has been overwritten
...
// 
// Project: "LibrainianCore", File: "Duration.cs" was last formatted by Protiguous on 2020/03/16 at 3:08 PM.
...
namespace Librainian.Measurement.Time {
    ...
    public struct Duration : IComparable<Duration>, IComparable<TimeSpan> {
        ...
        public Duration( [NotNull] params TimeSpan[] times ) {
            if ( times is null ) {
                throw new ArgumentNullException( paramName: nameof( times ) );
            }

            var totalMilliseconds = times.Where( predicate: span => span != default ).Sum( selector: timeSpan => timeSpan.TotalMilliseconds );

            this.Microseconds = totalMilliseconds * MicrosecondsPerMillisecond;
        }

        public static Duration FromDays( Double value ) => new Duration( days: new Days( value: ( Rational ) value ) );

        public static Duration FromHours( Double value ) => new Duration( hours: new Hours( value: ( Rational ) value ) );

        public static Duration FromMicroseconds( Double value ) => new Duration( microseconds: new Microseconds( value: ( Rational ) value ) );

        public static Duration FromMilliseconds( Double value ) => new Duration( milliseconds: new Milliseconds( value: value ) );

        public static Duration FromMinutes( Double value ) => new Duration( minutes: new Minutes( value: ( Rational ) value ) );

        public static Duration FromSeconds( Double value ) => new Duration( seconds: new Seconds( value: ( Rational ) value ) );

        public static Duration FromTicks( Int64 value ) => new Duration( ticks: value );

        public static Duration FromWeeks( Double value ) => new Duration( weeks: new Weeks( weeks: ( Rational ) value ) );

        public static Duration FromYears( Double value ) => new Duration( years: new Years( value: ( Rational ) value ) );
        ...
        /// <returns></returns>
        public Int32 CompareTo( Duration other ) => this.Microseconds.CompareTo( value: other.Microseconds );
        ...
        /// <param name="other"></param>
        /// <returns></returns>
        public Int32 CompareTo( TimeSpan other ) => this.TotalMilliseconds().CompareTo( value: other.TotalMilliseconds );
