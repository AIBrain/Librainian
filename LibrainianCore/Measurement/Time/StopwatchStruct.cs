...
// 
// This source code contained in "StopwatchStruct.cs" belongs to Protiguous@Protiguous.com unless otherwise specified or the original license has been overwritten
...
// 
// Project: "LibrainianCore", File: "StopwatchStruct.cs" was last formatted by Protiguous on 2020/03/16 at 3:08 PM.
...
namespace Librainian.Measurement.Time {
    ...
    /// <summary>
    /// A value-type implementation of Stopwatch. It allows for high precision benchmarking without needing to instantiate reference types on the managed heap. Because of native
    /// calls to QueryPerformanceCounter, this struct will only work on Windows. Note: because this is a struct, instances will be passed by value. This means every time you assign an
    /// instance to another variable, or pass it as a parameter, a copy is made. Calling methods like Stop() on one copy, will not have any effect on other copies. Unless you are
    /// comfortable with the implications of a pass-by-value mutable struct, it is highly recommended that you only assign each instance to one variable, and do not pass it to other
    /// methods.
    /// </summary>
    ...
    public struct StopwatchStruct {
        ...
        /// <summary>Gets the total elapsed time that the stopwatch has run. This property is accurate, even while the stopwatch is running.</summary>
        public TimeSpan Elapsed => TimeSpan.FromMilliseconds( this.GetElapsedMilliseconds() );

        /// <summary>The number of "counts" that the stopwatch has run. This property is accurate, even while the stopwatch is running.</summary>
        ...
        /// <summary>Returns the total number of milliseconds that the stopwatch has run. This property is accurate, even while the stopwatch is running.</summary>
